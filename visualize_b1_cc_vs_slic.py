...
For a single SEVIR event, this script:
  ...
  3. Computes boundary Precision / Recall / F1_bound (eq. B1, Step 3) between
...
def boundary_f1(boundary_cells, boundary_slic, tolerance_px=2):
    """
    Precision / Recall / F1 at a given pixel tolerance, per B1 Step 3.
    Uses a disk structuring element for a proper (not cross-shaped) dilation.
    """
    se = disk(tolerance_px)
    dilated_cells = binary_dilation(boundary_cells, structure=se)
    dilated_slic = binary_dilation(boundary_slic, structure=se)

    n_slic = boundary_slic.sum()
    n_cells = boundary_cells.sum()

    precision = (boundary_slic & dilated_cells).sum() / (n_slic + 1e-8)
    recall = (boundary_cells & dilated_slic).sum() / (n_cells + 1e-8)
    f1 = 2 * precision * recall / (precision + recall + 1e-8)
    return float(precision), float(recall), float(f1), dilated_cells, dilated_slic
...
def render_match_miss_panel(vil_phys, boundary_cells, boundary_slic,
                            dilated_cells, dilated_slic):
    """
    Diagnostic overlay attributing precision/recall failures to pixels:
      green   = SLIC boundary AND within tolerance of a cell boundary (TP)
      orange  = SLIC boundary but NOT within tolerance  (false positive -> hurts precision)
      magenta = cell boundary but NOT within tolerance of any SLIC boundary (false negative -> hurts recall)
    """
    H, W = vil_phys.shape
    panel = np.tile((vil_phys / (vil_phys.max() + 1e-6))[:, :, None], (1, 1, 3)) * 0.35

    tp_mask = boundary_slic & dilated_cells
    fp_mask = boundary_slic & ~dilated_cells
    fn_mask = boundary_cells & ~dilated_slic

    panel[tp_mask] = [0.0, 0.9, 0.0]
    panel[fp_mask] = [1.0, 0.55, 0.0]
    panel[fn_mask] = [0.95, 0.0, 0.85]
...
def visualize_event(event_id, vil_phys, cell_map, M, fused_frame0,
    ...
    for i, n_req in enumerate(n_list):
        ...
        precision, recall, f1, dil_cells, dil_slic = boundary_f1(
            boundary_cells, boundary_slic, tolerance_px=tolerance_px
        )
        ...
        match_panel = render_match_miss_panel(
            vil_phys, boundary_cells, boundary_slic, dil_cells, dil_slic
        )
        axes[row, 2].imshow(match_panel, origin="lower")
        axes[row, 2].set_title(
            f"P={precision:.3f}  R={recall:.3f}  F1={f1:.3f}  (tol={tolerance_px}px)",
            fontsize=10,
        )
        axes[row, 2].axis("off")

    legend_text = (
        "Match/miss panel:  green = matched boundary (TP)   "
        "orange = spurious SLIC boundary (hurts precision)   "
        "magenta = missed cell boundary (hurts recall)"
    )
    ...
    if len(n_list) > 1:
        ...
        ax2.plot(ns, precs, "b--", label="Precision", alpha=0.7)
        ax2.plot(ns, recs, "r--", label="Recall", alpha=0.7)
        ax2.set_xlabel("N (superpixels)")
        ax2.set_ylabel("Score")
        ax2.set_title(f"F1_bound vs N — Event {event_id}")
...
def run(event_id, n_list, tau_key, sigma_cell, tolerance_px,
       split_merged, n_max_sweep, use_dem, sigma_sweep, out_dir):
    ...
    f1_per_n = visualize_event(
        ...
        n_list, tau_cell, sigma_cell, tolerance_px, split_merged, out_dir,
    )
