...
# ─────────────────────────────────────────────────────────────────────────────
# HPC THREAD-PINNING GUARD  (new — cluster-sweep efficiency, no numerics change)
# ─────────────────────────────────────────────────────────────────────────────
# run_benchmark() already parallelises across events with a ProcessPoolExecutor
# (see _benchmark_event_worker). Left unset, each of those W worker processes
# would ALSO have numpy/scipy's BLAS and JAX's XLA host backend each try to
# claim every core on the node, so W workers x all-cores-per-worker massively
# oversubscribes whatever ncpus a PBS chunk actually grants and slows every
# worker down together. Pinning each process to a single math-backend thread
# BEFORE numpy/jax are imported makes --workers (matched 1:1 to #PBS -l
# select=1:ncpus=N) the sole source of parallelism, which is what the rest of
# this file already assumes. setdefault() only fills in variables the user
# hasn't already exported, so a local single-process run, or a PBS script
# that deliberately sets these differently, is unaffected.
...
def run_benchmark(
        ...
        load_calibration_path: Optional[str] = None,
    ...
    2. Stratified sample: n_events_per_class per lifecycle class (A.6),
       or EVERY event in the catalogue when `all_events=True` (cluster
       sweep mode — step 2 becomes a no-op subsample and every event is
       loaded/benchmarked; classes are unaffected, only the per-class cap
       is removed).
    ...
    7. (Optional) A.3 tolerance calibration.
       Steps 4-7 are SKIPPED (loaded instead) when `load_calibration_path`
       points at an existing block_a_calibration.json — this is what lets
       a cluster sweep split into a one-time calibration job followed by
       many independent N-value jobs that all reuse identical constants
       instead of each recomputing them (calibration is deterministic
       given the same calibration event, so re-deriving it in every job
       would be wasted, non-parallelisable wall time, not a numerics
       change either way).
    8. For each event x N x seed: run IMEX and DOPRI5, record NFE.
       Skipped entirely when `calibrate_only=True` (used by the one-time
       calibration job, which only needs steps 1-7 to produce the JSON).
    ...
    # ── A.6: stratified sample (or ALL events, for a full cluster sweep) ───
    if all_events:
    ...
    # ── HPC cluster-sweep fast path: reuse a calibration JSON produced by an
    #     earlier `calibrate_only=True` run instead of recomputing steps
    #     4-7 (c_sigma / D / per-class sigma_init / tolerance) in every one
    #     of the (potentially dozens of) N-value array jobs. Calibration is
    #     deterministic given the same calibration event, seed=42 sampling,
    #     and TARGET_RHO table, so this changes nothing about the numbers —
    #     it only avoids recomputing the identical constants once per job. ──
    loaded_calibration = None
    if load_calibration_path and os.path.exists(load_calibration_path):
        with open(load_calibration_path) as f:
            loaded_calibration = json.load(f)
        log.info(f"[calibration] Loaded precomputed calibration from "
                 f"{load_calibration_path} (calibration_event_id="
                 f"{loaded_calibration.get('calibration_event_id')})")

    if loaded_calibration is not None:
        c_sigma     = loaded_calibration["c_sigma"]
        D           = loaded_calibration["D"]
        class_sigma = dict(loaded_calibration["class_sigma_init"])
        rtol        = loaded_calibration["rtol"]
        atol        = loaded_calibration["atol"]
        log.info(f"[calibration] Using loaded c_sigma={c_sigma}  D={D:.6g}  "
        ...
        }, path=load_calibration_path or CALIB_JSON)

    if calibrate_only:
        log.info("[calibrate-only] Calibration complete; skipping main sweep "
                 "as requested (--calibrate-only).")
        return pd.DataFrame()
...
def main():
    ...
    parser.add_argument("--all-events", action="store_true",
                        help="Ignore --n_events and use EVERY event in "
                             ...
                             "cap). Intended for full cluster sweeps; local/"
                             "default behaviour is unaffected unless passed.")
    parser.add_argument("--calibrate-only", action="store_true",
                        help="Run only the calibration steps (c_sigma, D, "
                             "per-class sigma_init, tolerance), save "
                             "block_a_calibration.json (or --calib-json path), "
                             "and exit before the main event x N x seed sweep. "
                             "Intended as the one-time calibration job that "
                             "precedes a PBS array job over N values.")
    parser.add_argument("--calib-json", type=str, default=None,
                        help="Path to read/write the calibration JSON. With "
                             "--calibrate-only, this is where the calibration "
                             "is WRITTEN. Otherwise, if the file already "
                             "exists, calibration steps 4-7 are SKIPPED and "
                             "loaded from it instead of being recomputed — "
                             "use this to fan a single calibration out across "
                             "many independent per-N array-job tasks.")
    ...
    if args.all_events:
        log.info("Events: ALL catalogue events (--all-events)")
    else:
    ...
    results_df = run_benchmark(
        ...
        load_calibration_path=args.calib_json,
        calibrate_only=args.calibrate_only,
    )

    if args.calibrate_only:
        log.info("Calibration-only run complete.")
        return
