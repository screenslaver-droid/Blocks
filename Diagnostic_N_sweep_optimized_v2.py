...
import logging
import os
# Force C++ backend libraries to run on a single thread to prevent deadlocks
# when combined with Python's ThreadPoolExecutor.
...
def _safe_dem_save(disk_path, result):
    """
    Write DEM array directly to disk_path.

    No tmp file needed: fetch_and_regrid_dem holds a per-key lock so only
    one thread ever reaches this point for a given extent. np.save is wrapped
    in try/except so a failure is non-fatal — the in-memory cache still works.
    """
    try:
...
def fetch_and_regrid_dem(extent, nx=384, ny=384):
    ...
    # L1: in-memory (check before acquiring lock for speed)
    if key in _dem_cache:
        return _dem_cache[key]

    # One lock per unique extent — prevents multiple threads from fetching
    # and writing the same DEM file simultaneously (root cause of WinError 2).
    with _get_dem_lock(key):
        ...
        # Save to disk (non-fatal if it fails — in-memory cache still works)
...
class TemporalSLIC_DEM:
    ...
    def segment(self, fused_frame, precomputed_flow=None):
        ...
        if self.prev_labels is None:
            ...
            # USE PRECOMPUTED FLOW INSTEAD OF RECALCULATING
...
def sweep_event(event_id: str, catalog: pd.DataFrame,
    ...
    if resume and os.path.exists(out_path):
        try:
            ...
            # Complete = all N rows present + at least one metric value populated.
            # notna().any() not .all(): a valid SLIC run on a near-uniform frame
            # can legitimately produce NaN metrics for some N values.
            has_metrics = (
                "intra_sp_var_mean" in existing.columns
                and existing["intra_sp_var_mean"].notna().any()
            )
            if completed_n == expected_n and has_metrics:
    ...
    # ── PRECOMPUTE OPTICAL FLOW (Do this ONCE per event, not 7 times) ──────
    log.info(f"  {event_id}: Pre-computing optical flow...")
    precomputed_flows = [np.zeros((H, W, 2), dtype=np.float32)] # t=0 has no flow
    for t in range(1, T):
...
def main():
    ...
    parser.add_argument("--results_dir", type=str, default=None,
                        help="Override output directory. Use separate dirs for "
                             "concurrent runs to avoid file contention.")
