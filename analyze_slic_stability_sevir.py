...
USE_OPTICAL_FLOW = False   # Enable optical flow tracking (set False for standard SLIC)
...
def calculate_segmentation_iou(labels1, labels2):
    """
    Calculate mean Intersection over Union (IoU) between two segmentations.
    
    This measures how well the segmented regions overlap between consecutive frames.
    
    Algorithm:
    1. For each segment in labels1, find the best matching segment in labels2
       (the one with maximum overlap)
    2. Calculate IoU for each matched pair
    3. Return the mean IoU across all segments
    
    IoU = |Segment1 ∩ Segment2| / |Segment1 ∪ Segment2|
    
    Higher values indicate more stable/consistent segmentations.
    
    Args:
        labels1: Segmentation label map from frame t (H, W)
        labels2: Segmentation label map from frame t+1 (H, W)
        
    Returns:
        mean_iou: Mean IoU across all segment pairs (0 to 1)
    """
    # Get unique labels (excluding background 0)
    unique_labels1 = np.unique(labels1)
    unique_labels1 = unique_labels1[unique_labels1 != 0]
    
    if len(unique_labels1) == 0:
        return 1.0  # No segments - perfect match
    
    ious = []
    
    # For each segment in frame t
    for label1 in unique_labels1:
        # Create mask for this segment
        mask1 = (labels1 == label1)
        
        # Find which labels in labels2 overlap with this segment
        overlapping_labels2 = labels2[mask1]
        if len(overlapping_labels2) == 0:
            continue
            
        # Find the most common overlapping label (best match)
        # Exclude background (0)
        overlapping_labels2 = overlapping_labels2[overlapping_labels2 != 0]
        if len(overlapping_labels2) == 0:
            ious.append(0.0)  # Segment disappeared
            continue
            
        # Get the best matching label (most frequent overlap)
        unique, counts = np.unique(overlapping_labels2, return_counts=True)
        best_match_label2 = unique[np.argmax(counts)]
        
        # Calculate IoU between this pair
        mask2 = (labels2 == best_match_label2)
...
def analyze_event(event_id, catalog, use_flow=True, frame_skip=1, n_segments=150):
    """
    Analyze temporal stability of an event using Intersection over Union (IoU).
    ...
    Returns:
        results: Dict mapping channel names to lists of IoU values
    """
    # Get event metadata
    ...
    for ch in channels:
        ...
        # Run Temporal SLIC with IoU metric
        tslic = TemporalSLIC(
        ...
        for t in tqdm(frame_indices, desc=f"  {ch.upper()}", leave=False):
            ...
            if prev_labels is not None:
                iou = calculate_segmentation_iou(prev_labels, current_labels)
                iou_scores.append(iou)
            
            prev_labels = current_labels
        
        results[ch] = iou_scores
        
        if iou_scores:
            mean_iou = np.mean(iou_scores)
            std_iou = np.std(iou_scores)
            print(f"  Segmentation IoU: {mean_iou:.4f} ± {std_iou:.4f}")
            print(f"  Range: [{np.min(iou_scores):.4f}, {np.max(iou_scores):.4f}]")
        else:
            print(f"  [Warning] No IoU scores computed")
...
if __name__ == "__main__":
    ...
    print("Metric: Intersection over Union (IoU)")
    print("=" * 70)
    ...
    if results:
        ...
        for ch, iou_scores in results.items():
            if len(iou_scores) > 0:
                mean_iou = np.mean(iou_scores)
                std_iou = np.std(iou_scores)
                ...
                x_values = np.arange(len(iou_scores)) * FRAME_SKIP + 1
                ...
                plt.plot(
                    ...
                    marker='o', 
                    markersize=4,
                    label=f"{ch.upper()} (μ={mean_iou:.3f}, σ={std_iou:.3f})",
        ...
        # Save plot
        output_filename = f'iou_stability_{USE_OPTICAL_FLOW}_{event_id}.png'
        output_path = os.path.join(r"C:\Users\Siddharth Nair\OneDrive\Desktop\BTP-2\GAT-ODE\SEVIR", output_filename)
